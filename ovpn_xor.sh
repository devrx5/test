#!/bin/bash
#
# Thanks for https://github.com/Nyr
#


# Detect Debian users running the script with "sh" instead of bash
if readlink /proc/$$/exe | grep -q "dash"; then
	echo 'Этот установщик нужно запускать с помощью "bash", а не "sh".'
	exit
fi
...
if [[ $(uname -r | cut -d "." -f 1) -eq 2 ]]; then
	echo "В системе установлено старое ядро, несовместимое с этим установщиком."
	exit
fi
...
if grep -qs "ubuntu" /etc/os-release; then
	...
	echo "Похоже, что этот установщик работает в неподдерживаемом дистрибутиве.
...
if [[ "$os" == "debian" && "$os_version" -lt 9 ]]; then
	echo "Для использования этого установщика требуется Debian 9 или выше.
Эта версия Debian слишком старая и не поддерживается."
	exit
fi

if [[ "$os" == "centos" && "$os_version" -lt 7 ]]; then
	echo "Для использования этого установщика требуется CentOS 7 или выше.
...
if ! grep -q sbin <<< "$PATH"; then
	echo '$PATH не включает sbin. Попробуйте использовать «su -» вместо «su».'
	exit
fi
...
if [[ ! -e /dev/net/tun ]] || ! ( exec 7<>/dev/net/tun ) 2>/dev/null; then
	echo "В системе нет доступного устройства TUN.
...
obfuscate=$(openssl rand -base64 16)
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "Готово!"
	echo
	echo "Конфигурация клиента доступна в:" /var/www/html/"$client.ovpn"
	echo "Новые клиенты могут быть добавлены повторным запуском этого сценария."
else
	clear
	...
	case "$option" in
		1)
			...
			new_client
			echo
			echo "$client добавлен. Конфигурация доступна в:" /var/www/html/"$client.ovpn"
			exit
		;;
		2)
			...
			echo
			echo "Выберите клиента, которого хотите удалить:"
			...
			echo
			read -p "Подтвердите удаление $client ? [y/N]: " revoke
